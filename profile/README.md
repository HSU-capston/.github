...
분석 서버는 Python 기반 Flask 프레임워크로 구성되며, YOLOv11 기반 Pose Estimation 모델을 통해 사용자의 투구 영상을 분석하고 결과를 생성한다. 분석된 결과는 AWS S3에 저장되며, 결과 스코어 및 관절 분석 정보는 사용자에게 제공된다.

한편, Spring Boot 기반 서버는 사용자 인증, 분석 히스토리 저장 및 조회, 사용자 레벨 관리 등의 기능을 담당한다. Spring Boot는 모바일에서 들어온 분석 요청을 Flask 서버로 전달하고, 분석 결과 및 히스토리 관련 데이터는 Spring Boot 서버와 통신하여 연동된다.
